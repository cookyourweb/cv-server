...
# ── El prompt del CV ──────────────────────────────────────────────────────────
# 30-ago-2026. Estaba dentro de `/generar-cv` como f-string de 133 lineas. Ningun
# test podia leerlo, y ya mordio: pedia `2 full paragraphs (4-6 lines each)` y
# salian resumenes de 180 palabras (arreglado el 29-ago leyendolo a mano).
#
# Las reglas de aqui abajo son meses de correcciones sobre CVs reales. Un refactor
# que se lleve una por delante NO da error: da un CV peor. Por eso hay tests que
# las fijan, en tests/test_prompt_cv.py.
#
# Es `.format()`, no f-string: la plantilla tiene que existir sin las variables.
PROMPT_CV = """Act as a senior tech recruiter who screens 200+ CVs daily. Adapt this candidate's CV for a specific job offer.
